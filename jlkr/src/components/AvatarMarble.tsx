...
export const Avatar = ({ size, name, initials, square = false, rounded = false }: AvatarProps) => {
  ...
  const rx = square ? 0 : rounded ? SIZE * 0.2 : SIZE;
  const fontSize = displayInitials.length === 1 ? SIZE * 0.4 : SIZE * 0.32;
  ...
  return (
    ...
    >
      ...
      <mask
        ...
        height={SIZE}
      >
        <rect width={SIZE} height={SIZE} rx={rx} fill="white" />
      </mask>
      <g mask={`url(#${maskId})`}>
