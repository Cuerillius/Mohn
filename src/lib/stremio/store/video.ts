...
export function createVideoStore() {
	...
	const videoState = writable<VideoPlayerState>(initialState);
	...
	return {
		subscribe: videoState.subscribe,
		init: async (el: HTMLDivElement) => {
			videoContainer = el;

			videoCore = await getVideo();
