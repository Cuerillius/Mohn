<script lang="ts">
	...
	onMount(() => {
		...
		const unsubscribe = stream.subscribe((s) => {
			if (s) video.loadStream(s);
		});
...
<div
	class="absolute right-4 bottom-4 left-4 z-50 flex flex-col gap-2 rounded-xl border border-white/10 bg-black/60 p-3 shadow-2xl backdrop-blur-xl"
>
	<Seekbar
